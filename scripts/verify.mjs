...
// 日本語本文の検査。**配られた検査器は、呼ばれなければ何も守らない**(HC-183)。
// 共通の text_hygiene は src/data を既定で飛ばすので、ここから明示のファイル引数で渡す(HC-121)
runStep("prose", "python scripts/prose_check.py");

if (!fast) {
  runStep("build", "npm run -s build");
  // 出荷物の検査は build の後にしか走らせられない(TEST_SPEC 実行規約)
  runStep("validation", "npx vitest run --config vitest.validation.config.ts");
} else {
  result.build = "skipped";
  result.validation = "skipped";
  console.log("  [skipped] build (--fast)");
  console.log("  [skipped] validation (--fast)");
}
